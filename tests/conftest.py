...
def generate_chrom_pos(seed=42):
...
def generate_ref(salt=42):
    for chrom, pos in generate_chrom_pos():
...
def generate_probes(salt=42):
    for chrom, pos, ref in generate_ref():
...
def generate_header_lines(num_snps=730059, num_samples=24):
    yield "[Header]"
...
def generate_data_lines(samples, probes):
    yield "[Data]"
    header = [
        "Sample ID",
        "RsID",
    ...
    for i, sample in enumerate(samples, 1):
        ...
        for probe in probes:
            ...
            yield map_to_line(header, data)


def generate_lines(samples=["sample1"]):
    # first generate the probes
    probes = tuple(generate_probes())
    # now we know how many probes and how many samples we have can generate header
    for line in generate_header_lines(num_snps=len(probes), num_samples=len(samples)):
        yield line

    # now we can generate data lines for each sample
    for line in generate_data_lines(samples, probes):
        yield line


@pytest.fixture
def fixture_illumina_lines():
    with StringIO("\n".join(generate_lines())) as illumina:
        yield illumina
